...
int main(void)
{
	...
	// ...
	// 멀티캐스트 주소로 전송된 데이터(멀티캐스트 데이터) 수신

	// 1. UDP bind
	memset( &adr, 0, sizeof(adr) );
	adr.sin_family = AF_INET;
	adr.sin_addr.S_un.S_addr = htonl(INADDR_ANY); //inet_addr("192.168.0.2");
	...
	ret = setsockopt(	hRecvSock, IPPROTO_IP, IP_ADD_MEMBERSHIP, 
						(void*)&joinAdr, sizeof(joinAdr) );
	if (ret == SOCKET_ERROR) {
